...
BlocksWithPartition MergeTreeDataWriter::splitBlockIntoParts(
    Block && block, size_t max_parts, const StorageMetadataPtr & metadata_snapshot, ContextPtr context, DeduplicationInfoPtr deduplication_info)
{
    ...
    if (!metadata_snapshot->hasPartitionKey()) /// Table is not partitioned.
    {
        ...
        result[0].deduplication_info = deduplication_info->cloneSelf();
        result[0].partition_id = result[0].partition.getID(metadata_snapshot->getPartitionKey().sample_block);
        result[0].deduplication_info->rememberPartitionChoise(result[0].partition_id);
        return result;
    }
    ...
    {
        item.deduplication_info = deduplication_info->cloneSelf();
        item.partition_id = item.partition.getID(metadata_snapshot->getPartitionKey().sample_block);
        item.deduplication_info->rememberPartitionChoise(item.partition_id);
    }
