...
void ReplicatedMergeTreeSink::finishDelayedChunk(const ZooKeeperWithFaultInjectionPtr & zookeeper)
{
    ...
    for (auto & partition : delayed_chunk->partitions)
    {
        ...
        while (true)
        {
            ...
            auto block_ids = partition.block_with_partition.deduplication_info->getBlockIds(partition.block_with_partition.partition_id, deduplicate);
...

            {
                ...
                auto result = partition.block_with_partition.deduplication_info->deduplicateBlock(
                    conflict_block_ids,
                ...
                LOG_DEBUG(
                    log,
                    "After filtering by collision, removed {}, was {}, left {} rows in the inserted block",
                    result.removed_count,
                    partition.block_with_partition.block.rows(),
                    result.filtered_block->rows());

                partition.block_with_partition.block = std::move(*result.filtered_block);
                partition.block_with_partition.deduplication_info = std::move(result.deduplication_info);
            }
    ...
    size_t replicas_num)
{
    ...
    auto get_logs_ops = [&] (Coordination::Requests & ops)
    {
        ...
        {
            LOG_DEBUG(log, "ReplicatedMergeTreeSink::commitPart commit async {} deduplicate {}", is_async_insert, deduplicate);
